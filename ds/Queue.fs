...
type Queue<'T> private (theWorking:'T list, theFront:'T list Lazy, fLength, theRear:'T list, rLength) =
    ...
    let ensureFront() = match rLength > fLength with
    ...
    let (working:'T list, front:'T list Lazy, frontLength, rear:'T list, rearLength) = ensureFront() |> ensureWorking
    ...
    member this.Enqueue value = 
        new Queue<'T>(working, front, frontLength, value::rear, rearLength+1)

    new (vals:'T list) = 
        new Queue<'T>(vals, lazy([]), 
            List.length vals, 
            [], 0)
