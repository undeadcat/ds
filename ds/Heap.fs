...
type Node<'T when 'T:equality>(nodes:Node<'T> list, value:'T) =  
    ...
    member this.Equals (node:Node<'T>) =
        this.value = node.value && this.nodes = node.nodes
...
type Heap<'T when 'T:equality> private (roots: Option<Node<'T>> list, size:int, comparer:'T -> 'T-> bool) =
    ...
    let getHeadNode () =
        let findMin (x:Node<'T>) (y:Node<'T>) = if comparer x.value y.value then x else y
    ...
    member this.Add value =
        newHeap(mergeInternal [Some(Node([],value))], size + 1)
